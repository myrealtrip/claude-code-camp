...
│       │          v                No                   │
│       │     3'. Re-fix             │                   │
│       │     (new strategy)  iteration < max?──No──>Stop│
│       │          │                 │                   │
│       └──────────┴─────────── Yes ─┘                   │
└────────────────────────────────────────────────────────┘
```
...
이 스킬의 동작 방식대로:
- 페르소나별로 **Agent tool**을 `run_in_background: true`, **`model: "sonnet"`**으로 병렬 실행
- 각 Agent가 `claude -p` multi-turn 대화 실행
- 대화 완료 후 **conversation history를 `.reports/{RUN_ID}/conversation-persona-{id}.jsonl`에 저장**
- LLM-as-Judge 평가
- per-persona 리포트 생성

**대화 기록 저장**: 각 페르소나 Agent는 평가 직전에 JSONL에서 **assistant text + tool_use만 추출**하여 `.reports/{RUN_ID}/conversation-persona-{id}.md`에 저장합니다. `--verbose`의 raw 출력 전체가 아닌, 읽기 쉬운 대화 전문만 보존합니다.

```bash
# JSONL에서 assistant text + tool_use만 추출
for turn_file in /tmp/e2e-turn-{persona_id}-*.jsonl; do
  echo "=== Turn $(basename $turn_file) ===" >> .reports/{RUN_ID}/conversation-persona-{id}.md
  grep '"type":"assistant"' "$turn_file" | python3 -c "
import sys, json
for line in sys.stdin:
    ...
    for c in msg.get('message',{}).get('content',[]):
        if c.get('type') == 'text':
            ...
            print(f'[Tool: {c[\"name\"]}]')
  " >> .reports/{RUN_ID}/conversation-persona-{id}.md
done
```

**주의**: TeamCreate가 아닌 **Agent tool의 병렬 호출**을 사용합니다. 모든 페르소나에 대해 한 번의 메시지에서 여러 Agent tool을 동시에 호출하고, `run_in_background: true`로 설정하여 병렬 실행합니다. 모든 Agent 완료 알림을 대기합니다.

**모델 정책**:
- **페르소나 실행 Agent**: `model: "sonnet"` — 비용 효율적으로 multi-turn 대화 시뮬레이션
- **결과 분석 (Step 2)**: `model: "opus"` — 리포트 분석, regression 감지, 수정 계획 수립은 정확도가 중요
...
iteration 2 이상에서는 이전 iteration의 평균 점수와 비교합니다:

- **평균 점수가 이전보다 하락**하면 → **Regression 발생** → Step 4-3 Rollback으로 이동
- 평균 점수가 유지 또는 상승이면 → 정상 진행

Regression 판단 기준:
- `avg_score(current) < avg_score(previous)` 이면 regression
- 개별 criterion 점수가 아닌 **전체 평균**으로 판단 (일부 criterion이 내려가도 전체가 올라가면 OK)
...
### 4-2. Regression Rollback

Step 2-3에서 regression이 감지된 경우:
...
2. 콘솔에 경고 출력:
   ```
   ⚠️ Regression detected! (avg: {prev_avg} → {curr_avg})
      Reverted commit: {commit_hash}
      Retrying with different strategy...
   ```
3. **수정 전략을 변경**하여 Step 3을 재실행:
   - 이전에 수정했던 criterion은 제외하고 다른 부분을 수정
   - 이전 수정이 광범위했다면 더 보수적으로 (1-2개 Edit 이내)
   - 동일 criterion에서 2회 연속 regression 시 해당 criterion을 `--exclude`에 추가하고 다음으로 진행
4. 재수정 후 커밋하고 Step 1(재테스트)로 돌아감

**주의**: rollback은 iteration 카운트를 소모합니다. regression + 재시도로 1회 iteration이 사용됩니다.
