...
```bash
rm -f .reports/report-persona-*.md .reports/summary.md
```

이전 iteration의 리포트가 남아있으면 teammate가 재사용할 위험이 있으므로 **반드시 삭제**합니다.
...
모든 `.reports/report-persona-*.md` 파일을 읽어서 각 criterion별 점수를 추출합니다.
