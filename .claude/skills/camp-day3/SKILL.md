...
3. **심화 비교 테이블** — 세 개념의 차이를 3가지 축으로 보여준다:
...
- **퀴즈 진행**: `use camp-quiz` 패턴을 따르세요 — AskUserQuestion으로 선택지 출제, 사용자 답변 후 정답 공개, 맞으면 칭찬/틀리면 해설. **고급 사용자**(tech_literacy가 high 이상)에게는 퀴즈 정답 후 "보너스 질문: 이 상황에서 다른 방법을 쓸 수도 있을까요?" 같은 open-ended 심화 질문을 추가하세요.
- **섹션 전환**: `use camp-section-transition` 패턴을 **반드시** 따르세요 — 섹션 핵심 요약 후 **남은 시간을 반드시 명시적으로 안내**하고 다음 섹션 진행 여부를 확인. 남은 시간 안내를 생략하지 마세요. 각 섹션별 누적 시간 기준:
...
- **상호작용 품질 (중요)**: 일방적 강의가 되지 않도록 다음 체크포인트에서 반드시 AskUserQuestion을 사용하세요:
  - 각 개념 설명 후: "여기까지 이해되셨나요?" (선택지: 잘 이해됨 / 조금 헷갈림 / 다시 설명해주세요)
  - 각 실습 전: 난이도 선택 (가이드 모드 / 도전 모드)
  - 각 실습 후: 결과 확인 (잘 됐어요 / 에러남 / 안 보여요)
  - 각 퀴즈: 정답 선택
  - 각 섹션 끝: 다음 섹션 진행 여부
  - **"조금 헷갈림" 또는 "다시 설명해주세요" 선택 시**: 반드시 다른 비유나 예시로 재설명. 같은 설명 반복 금지.
  - **사용자 응답에 따라 진행을 분기하세요**: 예를 들어 "잘 이해됨"이면 심화 포인트 한 줄 추가, "헷갈림"이면 추가 비유 제공.
