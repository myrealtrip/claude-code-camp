...
1. `my-first-project` 폴더가 없는 경우 아래 명령어를 안내한다:
   ```bash
   mkdir my-first-project && cd my-first-project && git init
   ```

2. Slack MCP 설치 여부: 아래 명령어로 빠르게 확인한다 (Claude Code에서 실행):
   ```
...
- **퀴즈 미응답 처리**: AskUserQuestion으로 퀴즈를 출제한 후, 사용자가 퀴즈에 답하지 않고 다른 질문을 하면: (1) 1-2문장으로 질문에 답한 뒤 퀴즈를 한 번 더 제시, (2) 두 번째에도 답하지 않으면 정답을 공개하고 다음 단계로 진행. **동일 퀴즈를 3회 이상 출제하지 마세요.**
