...
2. **화면 분할 안내**: 실습 효율을 위해 터미널 창을 **좌우로 나란히** 띄우라고 안내한다:
   ...
   - macOS 기준: 창 상단 초록 버튼 길게 누르면 화면 분할 가능
   - 또는 터미널 탭/창을 하나 더 열어서 나란히 배치
3. **Bash 도구로 `uname -s`를 실행하여 사용자의 OS를 자동 감지**한다
   - `Darwin` → macOS
   - `Linux` → Linux
   - 그 외 → Windows
...
2. skill-creator 플러그인 설치를 안내한다:
   ...
   - 먼저 **플러그인 Scope(적용 범위)** 개념을 설명한다:
     ...
     - **user**: 내 모든 프로젝트에서 사용 (`~/.claude/plugins/`에 저장, 나만 사용)
     - **local**: 이 프로젝트에서 나만 사용 (`.claude/plugins/`에 저장, git에 포함 안 됨)
     - **project**: 이 프로젝트 전체에서 사용 (`.claude/plugins/`에 저장, 팀원과 공유됨)
...
2. **Google MCP 설치**: 이어서 아래 명령어를 입력하라고 안내한다:
   ```
   /plugin install google
   ```
   - scope는 **user**를 선택
   - 설치 과정에서 Google 계정 인증을 진행한다
...
2. **직접 체험**: 사용자에게 아래를 입력하라고 안내한다:
   ...
   ```
   - Claude가 `date` 명령을 실행하는 것을 확인하게 한다
...
> Bash 도구로 `open .claude/skills/camp-day2/orchestration-diagram.html`을 실행하여 오케스트레이션 도식 애니메이션을 브라우저에 띄운다. 설명하면서 탭을 전환하며 보여준다.
...
3. **Skill 배포 단계 정리**:
   ...
   | 단계 | 방법 | 비유 |
   |------|------|------|
   | 나만 쓰기 | `~/.claude/skills/` 또는 `.claude/skills/` | 내 레시피를 수첩에 적어두기 |
...
- 각 섹션이 끝나면 AskUserQuestion으로 "다음 섹션으로 넘어갈까요?"를 확인하세요. 이때 **남은 시간**을 함께 표시하세요. 각 섹션별 누적 시간 기준:
