...
<!--
[SKILL 구현 참고 — 이 주석은 강의에 표시되지 않음]

skill 이름: setup-check (가칭)

이 섹션의 환경 확인 및 설치는 skill이 자동으로 처리한다.
skill이 수행해야 할 동작:
...
0. OS 자동 감지 (가장 먼저 수행)
   - 감지 방법: `uname -s` 실행
     - "Darwin"  → macOS
     - "Linux"   → Linux (WSL 포함)
     - 실패 시   → `$env:OS` 확인 → "Windows_NT"이면 Windows
   - WSL 여부 추가 확인 (Linux인 경우):
     - `cat /proc/version` → "microsoft" 또는 "WSL" 포함 시 → Windows (WSL)
   - 감지 결과를 사용자에게 알려주기:
     - macOS → "macOS 환경이시네요! brew를 사용해서 설치를 진행합니다."
     - Windows (WSL) → "Windows(WSL) 환경이시네요!"
     - Windows (네이티브) → "Windows 환경이시네요! PowerShell 기준으로 안내합니다."
     - Linux → "Linux 환경이시네요!"
   - 이후 모든 안내는 감지된 OS에 맞는 내용만 보여주기 (불필요한 OS 안내 생략)

1. 환경 진단 (Bash tool로 직접 실행하여 확인)
   - AskUserQuestion으로 물어보지 말고, Bash tool로 아래 명령어를 직접 실행할 것:
     - Git: `git --version`
     - GitHub CLI: `gh --version`
     - GitHub 로그인 상태: `gh auth status`
     - Node.js: `node --version`
   - Git 사용자 설정도 직접 확인: `git config --global user.name`, `git config --global user.email`
   - 주의: Claude Code 자체는 이 skill의 점검 대상이 아님 (이미 실행 중이므로)
   - 각 명령어는 독립적이므로 가능하면 병렬로 실행할 것

2. 실행 결과를 요약하여 사용자에게 컨펌 받기
   - Bash 실행 결과를 ✅ 설치됨 (버전) / ❌ 미설치 형태로 요약 표시
   - 예시:
     🖥️ macOS 환경 감지
     ✅ Git 2.47.1
     ✅ GitHub CLI 2.67.0
     ❌ GitHub 로그인 안됨
     ✅ Node.js 22.0.0
   - AskUserQuestion으로 "위 결과가 맞나요? 미설치 항목을 설치하겠습니다." 확인
   - 사용자 승인 후 설치 진행

4. 설치 수행 (OS별 분기)
   - macOS: brew 우선 사용 (brew 없으면 brew부터 설치 확인)
     - Git: `brew install git`
     - GitHub CLI: `brew install gh`
     - Node.js: `brew install node`
   - Windows:
     - Git: https://git-scm.com/download/win 안내
     - GitHub CLI: https://cli.github.com 안내
     - Node.js: https://nodejs.org LTS 안내
   - Claude Code는 skill 범위 밖 (사용자가 이미 설치하고 실행한 상태에서 이 skill을 호출함)

5. 설치 후 설정
   - Git user.name / user.email 미설정 시 → 사용자에게 이름/이메일 입력받아 설정
   - GitHub CLI 로그인 안 되어 있으면 → `gh auth login` 실행 안내
   - GitHub 계정이 없으면 → https://github.com 가입 안내

6. 최종 확인
   - 모든 도구 재확인 후 체크리스트 출력
   - 전부 ✅이면 "환경 준비 완료!" 메시지
-->

먼저 Claude Code를 설치한 뒤, 나머지 도구는 Claude Code에게 맡기겠습니다.

### 필요한 도구

| 도구 | 역할 |
|------|------|
| **Git** | 파일 변경 이력 추적 — 되돌리기가 가능한 안전장치 |
| **GitHub CLI** (`gh`) | 터미널에서 GitHub 사용 — PR 생성, 이슈 관리 등 |
| **Node.js** | Claude Code 실행에 필요한 런타임 |
| **Claude Code** | 오늘의 주인공 |

### Step 1: Claude Code 설치

Claude Code는 직접 설치해야 합니다. 터미널을 열고 아래 명령어를 입력하세요:

```bash
npm install -g @anthropic-ai/claude-code
```

> `npm`이 없다는 에러가 나오면, 먼저 Node.js를 설치하세요:
> - **macOS**: `brew install node` (Homebrew가 없으면 [brew.sh](https://brew.sh) 에서 먼저 설치)
...
`claude x.x.x` 같은 출력이 나오면 성공입니다.

### Step 2: 나머지 환경 확인

Claude Code를 실행한 뒤, 아래와 같이 입력하세요:
...
Claude Code가 자동으로:
1. Git, GitHub CLI 등이 설치되어 있는지 **확인**하고
2. 결과를 **보여준 뒤**
3. 미설치 항목이 있으면 설치할지 **물어봅니다**

> 💡 GitHub 계정이 없으면 먼저 https://github.com 에서 가입하세요.

### 최종 체크리스트

> 💡 확인은 **새 터미널 창**을 열어서 하세요. Claude Code가 실행 중인 터미널과 별도로 열어야 합니다.

모든 설치가 끝나면 아래가 전부 통과해야 합니다:
...
Claude Code는 매번 새로운 대화를 시작할 때 CLAUDE.md를 읽고, 여기에 적어둔 지시사항을 따릅니다.

#### 규칙뿐 아니라 내 선호도 넣을 수 있다

CLAUDE.md에는 업무 규칙뿐 아니라 **나의 소통 선호**도 적을 수 있습니다:

```markdown
# 예시: CLAUDE.md에 이렇게 적으면
- 항상 한국어로 답변해주세요
- 비개발자도 이해할 수 있게 쉽게 설명해주세요
- 설명할 때 예시를 꼭 들어주세요
- 존댓말을 사용해주세요
```

→ Claude는 매 대화마다 이 규칙을 읽고 따릅니다. 내가 원하는 방식으로 일하는 AI 비서가 되는 거죠.

### 2-2. Scope — 어디에 두느냐에 따라 적용 범위가 달라진다

CLAUDE.md의 핵심은 **어디에 놓느냐**입니다.
...
CLAUDE.md는 **팀과 공유하는 것**과 **나만 쓰는 것**으로 나뉩니다.
